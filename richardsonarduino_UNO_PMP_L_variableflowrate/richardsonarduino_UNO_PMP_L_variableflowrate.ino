...
void setup() {
  ...
  analogWrite(PUMP_PWM_PIN, maxFlowRate);
  Serial.println("Pump started with default flow rate: " + String(maxFlowRate));
}
  ...
  if (isInputStringComplete) {                          // If a complete input string is received
    if (inputString.startsWith("F")) {                  // Check if the input string is a flow rate command (e.g., "F128")
      ...
      if (newFlowRate >= 0 && newFlowRate <= 255) {
        ...
        Serial.println("Invalid flow rate. Please enter a value between 0 and 255.");
      }
    } else {
